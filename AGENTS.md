...
- The design system is `@webtides/element-library` (Web Components), server-rendered through `@webtides/element-js-ssr-renderer` in `src/hooks.server.ts` and upgraded on the client by the root layout's `autoload` call

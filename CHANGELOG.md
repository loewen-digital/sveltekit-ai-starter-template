...
- Passwords are hashed with scrypt from `node:crypto` instead of `-rs/argon2`, so the app runs on Cloudflare Workers (`wrangler dev` and deploys were broken). Existing argon2 hashes no longer verify, re-register local users. The `prebuild` step and the wasm package are gone. See [decision 0001](docs/decisions/0001-scrypt-instead-of-argon2.md).
